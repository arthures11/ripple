...
func Sign(s string, hash []byte) ([]byte, error) {
	privKeyhere, _ := hex.DecodeString(s)

	return signECDSA(privKeyhere, hash)
...
func signECDSA(privateKey, hash []byte) ([]byte, error) {
	...
	//s, _ := verifyECDSA(priv.PubKey().SerializeCompressed(), sig.Serialize(), hash)
